...
**Translation features** ([cogs/translation.py](cogs/translation.py)): free Google Translate endpoint (no API key) via `aiohttp`. `,translate <texte>` is a hybrid command (ephemeral when invoked as a slash command). `,translator on <lang>` / `,translator off` opt the user into per-channel auto-translation: subsequent messages in that channel are translated and DM'd only to the subscribing user (never posted publicly).
